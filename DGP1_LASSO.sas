...
/**********************************  DGP1 avec independance linéaire **************************************/
...
proc glmselect data= table1 noprint outdesign= DGP1_results;
model Y =X1-X50 / selection=lasso(choose=AIC stop=SBC);
run;
proc transpose data=DGP1_results out=DGP1_results; run;
...
create table_AICC from res_AICC[colnames=colres];
...
create table_BIC from res_BIC[colnames=colres];
append from res_BIC;
close table_BIC;
...
create table_SBC from res_SBC[colnames=colres];
append from res_SBC;
close table_SBC;
...
create table_CP from res_CP[colnames=colres];
append from res_CP;
close table_CP;
...
create table_CV from res_CV[colnames=colres];
append from res_CV;
close table_CV;
...
create table_PRESS from res_PRESS[colnames=colres];
